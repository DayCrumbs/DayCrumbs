//
//  AppColours.swift
//  DayCrumbs
//
//  Created by Ibnu Taufick Ahraza on 7/15/26.
//

import SwiftUI

struct AppColour {
    static let bgPutih = Color("PutihDayCrumbs")
    static let bgKuning = Color("KuningDayCrumbs")
    static let btnKuning = Color("KuningDayCrumbs")
    static let btnPutih = Color("PutihDayCrumbs")
    static let btnCoklat = Color("CoklatDayCrumbs")
    static let chevPutih = Color("PutihChevDayCrumbs")
    static let chevKuning = Color("KuningDayCrumbs")
    static let txtCoklat = Color("CoklatDayCrumbs")
    static let txtBubbleCoklat = Color("CoklatTextDayCrumbs")
    static let txtPutih = Color("PutihDayCrumbs")
    static let cardKuning = Color("KuningDayCrumbs")
    static let bgMorning = Color("PlaceholderPagi")
    static let bgAfternoon = Color("PlaceholderSiang")
    static let bgEvening = Color("PlaceholderSore")
    static let bgNight = Color("PlaceholderMalam")
    static let barHappy = Color("Colour_Happy")
    static let barSad = Color("Colour_Sad")
    static let barAngry = Color("Colour_Angry")
    static let barSurprised = Color("Colour_Surprise")
    static let barFearful = Color("Colour_Fear")
    static let barDisgusted = Color("Colour_Disgust")
    
}
